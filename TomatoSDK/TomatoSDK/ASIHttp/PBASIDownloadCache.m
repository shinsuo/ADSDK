...
#import "PBASIDownloadCache.h"
#import "PBASIHTTPRequest.h"
#import <CommonCrypto/CommonHMAC.h>

static PBASIDownloadCache *sharedCache = nil;

static NSString *sessionCacheFolder = @"SessionStore";
static NSString *permanentCacheFolder = @"PermanentStore";
static NSArray *fileExtensionsToHandleAsHTML = nil;

@interface PBASIDownloadCache ()
+ (NSString *)keyForURL:(NSURL *)url;
- (NSString *)pathToFile:(NSString *)file;
@end

@implementation PBASIDownloadCache

+ (void)initialize
{
	if (self == [PBASIDownloadCache class]) {
...
- (void)updateExpiryForRequest:(PBASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
...
{
  return [PBASIHTTPRequest expiryDateForRequest:request maxAge:maxAge];
}

- (void)storeResponseForRequest:(PBASIHTTPRequest *)request maxAge:(NSTimeInterval)maxAge
{
	[[self accessLock] lock];
...
- (NSString *)pathToStoreCachedResponseDataForRequest:(PBASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (NSString *)pathToStoreCachedResponseHeadersForRequest:(PBASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (void)removeCachedDataForRequest:(PBASIHTTPRequest *)request
{
	[self removeCachedDataForURL:[request url]];
}

- (BOOL)isCachedDataCurrentForRequest:(PBASIHTTPRequest *)request
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
- (PBASICachePolicy)defaultCachePolicy
{
	[[self accessLock] lock];
	PBASICachePolicy cp = defaultCachePolicy;
	[[self accessLock] unlock];
	return cp;
}


- (void)setDefaultCachePolicy:(PBASICachePolicy)cachePolicy
{
	[[self accessLock] lock];
	if (!cachePolicy) {
...
- (void)clearCachedResponsesForStoragePolicy:(PBASICacheStoragePolicy)storagePolicy
{
	[[self accessLock] lock];
	if (![self storagePath]) {
		[[self accessLock] unlock];
...
+ (BOOL)serverAllowsResponseCachingForRequest:(PBASIHTTPRequest *)request
{
...
- (BOOL)canUseCachedDataForRequest:(PBASIHTTPRequest *)request
{
