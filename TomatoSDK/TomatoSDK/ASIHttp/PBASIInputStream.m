...
@implementation PBASIInputStream

+ (void)initialize
{
	if (self == [PBASIInputStream class]) {
		readLock = [[NSLock alloc] init];
	}
}

+ (id)inputStreamWithFileAtPath:(NSString *)path request:(PBASIHTTPRequest *)theRequest
{
	PBASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
	[theStream setStream:[NSInputStream inputStreamWithFileAtPath:path]];
	return theStream;
}

+ (id)inputStreamWithData:(NSData *)data request:(PBASIHTTPRequest *)theRequest
{
	PBASIInputStream *theStream = [[[self alloc] init] autorelease];
	[theStream setRequest:theRequest];
...
- (NSInteger)read:(uint8_t *)buffer maxLength:(NSUInteger)len
{
	...
	if ([PBASIHTTPRequest isBandwidthThrottled]) {
		toRead = [PBASIHTTPRequest maxUploadReadLength];
		if (toRead > len) {
			toRead = len;
		} else if (toRead == 0) {
			toRead = 1;
	...
	if (rv > 0)
		[PBASIHTTPRequest incrementBandwidthUsedInLastSecond:rv];
	return rv;
}
