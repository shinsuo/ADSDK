...
@class PBASIDataDecompressor;

extern NSString *PBASIHTTPRequestVersion;
...
extern NSString* const PBNetworkRequestErrorDomain;
...
extern unsigned long const PBASIWWANBandwidthThrottleAmount;

#if NS_BLOCKS_AVAILABLE
typedef void (^PBASIBasicBlock)(void);
typedef void (^PBASIHeadersBlock)(NSDictionary *responseHeaders);
typedef void (^PBASISizeBlock)(long long size);
typedef void (^PBASIProgressBlock)(unsigned long long size, unsigned long long total);
typedef void (^PBASIDataBlock)(NSData *data);
#endif

@interface PBASIHTTPRequest : NSOperation <NSCopying> {
	...
	id <PBASIHTTPRequestDelegate> delegate;
	...
	id <PBASIHTTPRequestDelegate, PBASIProgressDelegate> queue;
	...
	id <PBASIProgressDelegate> uploadProgressDelegate;
	...
	id <PBASIProgressDelegate> downloadProgressDelegate;
	...
	PBASIAuthenticationState authenticationNeeded;
	...
	PBASIDataDecompressor *dataDecompressor;
	...
	PBASIHTTPRequest *PACFileRequest;
	...
	PBASIBasicBlock startedBlock;

	//block to execute when headers are received
	PBASIHeadersBlock headersReceivedBlock;

	//block to execute when request completes successfully
	PBASIBasicBlock completionBlock;

	//block to execute when request fails
	PBASIBasicBlock failureBlock;

	//block for when bytes are received
	PBASIProgressBlock bytesReceivedBlock;

	//block for when bytes are sent
	PBASIProgressBlock bytesSentBlock;

	//block for when download size is incremented
	PBASISizeBlock downloadSizeIncrementedBlock;

	//block for when upload size is incremented
	PBASISizeBlock uploadSizeIncrementedBlock;

	//block for handling raw bytes received
	PBASIDataBlock dataReceivedBlock;

	//block for handling authentication
	PBASIBasicBlock authenticationNeededBlock;

	//block for handling proxy authentication
	PBASIBasicBlock proxyAuthenticationNeededBlock;
	
    //block for handling redirections, if you want to
    PBASIBasicBlock requestRedirectedBlock;
	#endif
}
...
@property (assign) PBASIHTTPRequest *mainRequest;
...
@property (assign, readonly) PBASIAuthenticationState authenticationNeeded;
...
@property (assign) id <PBASICacheDelegate> downloadCache;
@property (assign) PBASICachePolicy cachePolicy;
@property (assign) PBASICacheStoragePolicy cacheStoragePolicy;
