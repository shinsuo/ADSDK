...
@implementation PBASIAutorotatingViewController
...
@interface PBASIAuthenticationDialog ()
- (void)showTitle;
- (void)show;
...
@implementation PBASIAuthenticationDialog

#pragma mark init / dealloc

+ (void)initialize
{
	if (self == [PBASIAuthenticationDialog class]) {
		requestsNeedingAuthentication = [[NSMutableArray array] retain];
	}
}

+ (void)presentAuthenticationDialogForRequest:(PBASIHTTPRequest *)theRequest
{
...
- (UIViewController *)presentingController
{
	if (!presentingController) {
		presentingController = [[PBASIAutorotatingViewController alloc] initWithNibName:nil bundle:nil];
...
- (void)cancelAuthenticationFromDialog:(id)sender
{
	for (PBASIHTTPRequest *theRequest in [self requestsRequiringTheseCredentials]) {
...
- (NSArray *)requestsRequiringTheseCredentials
{
	...
	for (PBASIHTTPRequest *otherRequest in requestsNeedingAuthentication) {
...
- (void)presentNextDialog
{
	if ([requestsNeedingAuthentication count]) {
		PBASIHTTPRequest *nextRequest = [requestsNeedingAuthentication objectAtIndex:0];
