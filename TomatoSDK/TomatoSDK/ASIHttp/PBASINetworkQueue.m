...
- (void)dealloc
{
	...
	for (PBASIHTTPRequest *request in [self operations]) {
...
- (void)resetProgressDelegate:(id *)progressDelegate
{
	...
	if ([*progressDelegate respondsToSelector:selector]) {
		float value = 0.0f;
		[PBASIHTTPRequest performSelector:selector onTarget:progressDelegate withObject:nil amount:&value callerToRetain:nil];
	}
#endif
}
...
{
	if ([operation isKindOfClass:[PBASIHTTPRequest class]]) {
		
		PBASIHTTPRequest *request = (PBASIHTTPRequest *)operation;
...
- (void)addOperation:(NSOperation *)operation
{
	...
	if ([self showAccurateProgress]) {
		...
		if ([[request requestMethod] isEqualToString:@"GET"]) {
			if ([self isSuspended]) {
				PBASIHTTPRequest *HEADRequest = [request HEADRequest];
...
- (void)requestStarted:(PBASIHTTPRequest *)request
{
...
- (void)request:(PBASIHTTPRequest *)request didReceiveResponseHeaders:(NSDictionary *)responseHeaders
{
...
- (void)request:(PBASIHTTPRequest *)request willRedirectToURL:(NSURL *)newURL
{
...
- (void)request:(PBASIHTTPRequest *)request didReceiveBytes:(long long)bytes
{
	[self setBytesDownloadedSoFar:[self bytesDownloadedSoFar]+bytes];
	if ([self downloadProgressDelegate]) {
		[PBASIHTTPRequest updateProgressIndicator:&downloadProgressDelegate withProgress:[self bytesDownloadedSoFar] ofTotal:[self totalBytesToDownload]];
	}
}

- (void)request:(PBASIHTTPRequest *)request didSendBytes:(long long)bytes
{
	[self setBytesUploadedSoFar:[self bytesUploadedSoFar]+bytes];
	if ([self uploadProgressDelegate]) {
		[PBASIHTTPRequest updateProgressIndicator:&uploadProgressDelegate withProgress:[self bytesUploadedSoFar] ofTotal:[self totalBytesToUpload]];
	}
}

- (void)request:(PBASIHTTPRequest *)request incrementDownloadSizeBy:(long long)newLength
{
	[self setTotalBytesToDownload:[self totalBytesToDownload]+newLength];
}

- (void)request:(PBASIHTTPRequest *)request incrementUploadSizeBy:(long long)newLength
{
...
- (void)authenticationNeededForRequest:(PBASIHTTPRequest *)request
{
...
- (void)proxyAuthenticationNeededForRequest:(PBASIHTTPRequest *)request
{
...
{
	PBASINetworkQueue *newQueue = [[[self class] alloc] init];
